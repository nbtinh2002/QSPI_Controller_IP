...
#(
	...
	wire		hold_en, wp_en;
	wire 		enable, xip_en, quad_en, cpol, cpha, lsb_first, cmd_trigger;
	wire 		ce_busy,  ce_done;
	wire 		timeout, overrun, underrun, axi_err;
	wire 		err_set = timeout | overrun | underrun | axi_err ;
	wire [2:0]	clk_div;
	...
	qspi_csr #(.APB_ADDR_WIDTH(APB_ADDR_WIDTH)) csr_inst(
		...
		.xip_en_o(xip_en), 				.xip_active_i(1'b0),		.dma_done_set_i(dma_ce_done),		.cs_auto_o(cs_auto), 
		...
		.underrun_i(underrun), 
		.axi_err_i(axi_err),
		// FIFO_STAT					// FIFO signal
		.tx_level_i(tx_level),			.tx_full_i(tx_full), 
		.tx_empty_i(tx_empty),			.tx_data_o(tx_data), 
 		.rx_level_i(rx_level),			.tx_wen_o(tx_wen),  
		.rx_full_i(rx_full),			.rx_empty_i(rx_empty),
										.rx_data_i(rx_data),
										.rx_ren_o(rx_ren)	
	);
