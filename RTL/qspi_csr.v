...
//	output wire [7:0]	xip_write_op_o,
	...
	output wire			csr_tx_wen_o,
	output wire			csr_rx_ren_o
	);
	...
	reg cmd_done_reg, dma_done_reg, err_reg, fifo_tx_empty_reg, fifo_rx_full_reg;
	...
	always@(*) begin
		...
		if(read) begin
			case(paddr)
				...
				FIFO_RX_ADDR	:prdata = csr_data_i;
	...
	assign pready = !( (write && (paddr==FIFO_TX_ADDR) && tx_full_i) ||
	                  (read  && (paddr==FIFO_RX_ADDR) && rx_empty_i) );
...
//	assign wp_en_o		 		= ctrl_reg[11];
	...
	assign xip_write_en_o			= xip_cfg_reg[14];
...
//	assign xip_write_op_o		= xip_cmd_reg[15:8];
	...
	// FIFO data signals
	assign csr_data_o 			= (!tx_full_i && write && (paddr == FIFO_TX_ADDR)) ? pwdata : 32'b0;
	assign csr_tx_wen_o   			= !tx_full_i  && write && (paddr == FIFO_TX_ADDR);
	assign csr_rx_ren_o   			= !rx_empty_i && read  && (paddr == FIFO_RX_ADDR);	
endmodule